...
describe("SuccessState", () => {
  ...
  it("offers a booking button only for the Calendly URLs the product has, revealing the inline scheduler", () => {
    ...
    const video = screen.getByRole("button", { name: "Book a video call" });
    expect(video).toHaveAttribute("aria-expanded", "false");
    expect(screen.queryByRole("region", { name: "Book a video call" })).not.toBeInTheDocument();

    fireEvent.click(video);
    expect(video).toHaveAttribute("aria-expanded", "true");
    expect(screen.getByRole("region", { name: "Book a video call" })).toHaveAttribute("id", video.getAttribute("aria-controls"));
    ...
    expect(screen.queryByRole("button", { name: "Book a video call" })).not.toBeInTheDocument();
  ...
  it("tracks each scheduler as it opens and forwards only Calendly's own postMessage events", () => {
    const track = vi.fn();
    render(
      <SuccessState
        isRepeatCustomer={false}
    ...
    fireEvent.click(screen.getByRole("button", { name: "Book a video call" }));
