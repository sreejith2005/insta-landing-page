...
export class PreviewRepository implements FunnelRepository {
  ...
  private readonly idempotency = new Map<string, { inquiryId: string; customerId: string; isRepeatCustomer: boolean }>();
  ...
  async requestCallback(input: { inquiryId: string; sessionId: string; idempotencyKey: string }) {
    ...
    const callback = { callbackId: `cb_${randomUUID()}`, inquiryId: input.inquiryId, idempotencyKey: input.idempotencyKey };
