...
/**
 * Canonical column order, used only when a tab has no header row yet. Existing
 * sheets drive both reads and writes from their own header row, so columns may
 * be reordered or extended without touching this file.
 */
export const defaultHeaders = {
  products: [
    ...
    "calendly_store_url", "calendly_video_url", "whatsapp_number", "whatsapp_template",
    "whatsapp_enabled", "callback_enabled", "cta_order_json",
  ],
  customers: ["customer_id", "created_at", "name", "phone_normalized", "pin_code", "city", "first_source"],
  inquiries: [
    "inquiry_id", "customer_id", "created_at", "name", "phone_normalized", "pin_code", "city",
    "is_repeat_customer", "product_id", "reel_id", "campaign_id", "source", "utm_source",
    "utm_medium", "utm_campaign", "utm_content", "utm_term", "session_id",
    "landing_page_version", "idempotency_key",
  ],
  events: [
    "created_at", "event_name", "session_id", "inquiry_id", "customer_id", "product_id",
    "reel_id", "campaign_id", "source", "landing_page_version", "idempotency_key", "metadata_json",
  ],
  callbacks: ["callback_id", "inquiry_id", "session_id", "created_at", "idempotency_key", "status"],
} as const;
...
// A short sheet row leaves trailing columns absent, so every reader tolerates
// an undefined cell rather than assuming the column is present.
...
export function productFromRow(row: Row): ProductRecord | null {
  ...
  const whatsappNumber = /^[1-9][0-9]{7,14}$/.test((row.whatsapp_number ?? "").trim())
  ...
  return {
    ...
    calendly: {
      storeVisitUrl: httpsOrUndefined(row.calendly_store_url),
      videoConsultationUrl: httpsOrUndefined(row.calendly_video_url),
    },
    whatsapp: {
      number: whatsappNumber,
      messageTemplate: row.whatsapp_template || undefined,
    },
    ctas: {
...
export class GoogleSheetsRepository implements FunnelRepository {
  ...
  private async rows(tab: string) {
    ...
    if (headers.length) this.headerCache.set(tab, headers.map((header) => header.trim()));
  ...
  /**
   * Writes are header-indexed: each value is placed under its own column name,
   * so reordering columns in the spreadsheet cannot corrupt stored data.
   */
  ...
  async findByContext(context: ProductMapping) {
    ...
    const match = rows.find(
      (row) =>
        ...
        row.reel_id === context.reelId &&
        row.campaign_id === context.campaignId,
    );
    return match ? productFromRow(match) : null;
  }
  ...
  async acceptLead(input: LeadSubmissionInput) {
    ...
    // A repeat customer always produces a new inquiry row so the new product,
    // Reel, campaign, and attribution are never lost.
  ...
  async recordEvent(input: EventInput, idempotencyKey = "") {
  ...
  async requestCallback(input: { inquiryId: string; sessionId: string; idempotencyKey: string }) {
    ...
    await this.appendRecord(this.tabs.callbacks, defaultHeaders.callbacks, {
  ...
  async recordAppointment(input: AppointmentInput): Promise<AppointmentRecord | null> {
    const inquiries = await this.rows(this.tabs.inquiries);
    if (!inquiries.some((row) => row.inquiry_id === input.inquiryId)) return null;

    // Bookings are stored as attributable events; the idempotency column keeps
    // a retried Calendly callback from producing a second booking row.
    const events = await this.rows(this.tabs.events);
    const replay = events.find(
      (row) => row.event_name === "appointment_booked" && row.idempotency_key === input.idempotencyKey,
    );
    if (replay) return { appointmentId: replay.idempotency_key, inquiryId: replay.inquiry_id };
    ...
    return { appointmentId: input.idempotencyKey, inquiryId: input.inquiryId };
  }
}
