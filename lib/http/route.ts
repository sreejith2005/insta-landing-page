...
export async function readJsonBody(request: Request): Promise<unknown> {
  ...
  try { return JSON.parse(body); } catch { throw new RequestBodyError(400); }
}

export function guardMutationRequest(request: Request): Response | null {
  if (!request.headers.get("content-type")?.toLowerCase().startsWith("application/json")) return jsonError(415, "Please send a valid request.");
